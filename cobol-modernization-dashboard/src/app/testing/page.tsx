...
export default function TestingPage() {
  ...
  return (
    <AppShell
      title="Testing Agent"
      subtitle="Deep behavioral and structural validation for parser, JCL, conversion, and runtime-equivalence checks."
    >
      ...
      {!hydrated && (
        <div className="error-banner">Loading generated Java from the workspace...</div>
      )}

      {hydrated && !canRunTests && (
        <div className="error-banner">Generate Java code first from Single File, Conversion, or Project Upload to enable the full testing suite.</div>
      )}
      ...
      <div className="glass-card">
        <div className="panel-label" style={{ marginBottom: 12 }}>Test Detail Table</div>
                ...
                return (
                  <>
                    ...
                    >
                      ...
                      <td style={{ color: test.severity === "critical" ? "var(--danger)" : test.severity === "high" ? "var(--amber)" : "var(--text-muted)" }}>
                    ...
                    {isExpanded && detail && (
                      <tr key={`${test.id}-detail`}>
                        <td colSpan={5}>
                          <pre className="code-panel" style={{ minHeight: 80 }}>{typeof detail === "string" ? detail : JSON.stringify(detail, null, 2)}</pre>
                        </td>
                      </tr>
                    )}
                  </>
            ...
            {(report.behavioral_tests as TestResult[]).map((test) => (
              <div key={test.id} className="panel-card" style={{ borderColor: test.passed ? "rgba(52, 211, 153, 0.7)" : "rgba(239, 68, 68, 0.7)" }}>
                <div className="health-row">
                  ...
                  <StatusPill label={test.passed ? "Equivalent" : "Drift detected"} tone={test.passed ? "good" : "bad"} />
                </div>
                {test.stdout_diff?.length ? (
                  <div className="code-panel" style={{ minHeight: 100 }}>
                    {test.stdout_diff.map((diff, index) => (
                      <div key={index} style={{ display: "grid", gridTemplateColumns: "80px 1fr 1fr", gap: 12 }}>
                        <span style={{ color: "var(--text-muted)" }}>L{diff.line ?? index + 1}</span>
                        <span style={{ color: "var(--danger)" }}>{diff.cobol ?? ""}</span>
                        <span style={{ color: "var(--emerald)" }}>{diff.java ?? ""}</span>
                      </div>
                    ))}
                  </div>
                ) : null}
