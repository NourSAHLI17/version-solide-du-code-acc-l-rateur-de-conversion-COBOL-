...
- Metadata: **`analysis_engine`**, **`analysis_revision`**, reference to prompt doc for governance.
...
### Output 2 — Analysis JSON (semantic rollup, deterministic)

**Producer:** `AnalysisAgent` (`analysis_agent.py`).  
**Role:** Paragraph-level and program-level **meaning** for converters and humans—**grounded** in parser facts.

**Representative keys:**

| Key | Audience explanation |
|-----|---------------------|
| `analysis_engine` | Today: **`deterministic_python`** (repeatable). |
...
**Why it matters:** Converts “what does this paragraph do?” into **structured** answers without asking an LLM to re-parse COBOL.
...
| Config `PARSER_BACKEND` | `app/core/config.py` |
...
*Revision note: align `parser_revision` / `analysis_revision` strings with your deployed branch when presenting.*
