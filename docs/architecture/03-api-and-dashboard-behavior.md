...
| `POST /api/testing/behavioral-diff` | GnuCOBOL vs Java stdout comparison |
| `GET /api/testing/toolchain-status` | GnuCOBOL/Java toolchain availability |

Default backend port: **8010** (`start_backend.bat`). Config default in `AppConfig` is 8000 if `PORT` is unset.
