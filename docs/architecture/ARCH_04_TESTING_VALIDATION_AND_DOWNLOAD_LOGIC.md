...
## Behavioral Diff (production path)

`POST /api/testing/behavioral-diff` (`app/services/behavioral_diff_runner.py`) is the
primary behavioral verification path for ACME and single-file runs:

- Compiles and runs COBOL via GnuCOBOL (`cobc`)
- Compiles and runs Java via `javac` + `java`
- Compares stdout with layered scoring (`behavioral_layer_scoring_service.py`)
- Handles COPY prep, file harness staging, and toolchain status checks

Route: `app/api/routes/testing.py`.

## Behavioral Tests (testing agent)

The testing agent's behavioral tests use subprocess calls:
