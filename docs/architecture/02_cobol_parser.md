...
The factory returns one of three backends (`app/parsers/factory.py`):

| `PARSER_BACKEND` | Class |
|---|---|
| `hybrid` (default) | `HybridCobolParser` |
| `heuristic` | `ParserLayer` |
| `antlr` | `AntlrCobolParser` |

The heuristic core is `ParserLayer` in `app/parsers/cobol_parser.py`. Hybrid mode always
runs `ParserLayer` first, then merges ANTLR visitor output via `HybridMerger`.
